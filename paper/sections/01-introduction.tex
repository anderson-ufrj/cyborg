...
In prior work, we introduced the Developer Mental Model Framework (DMMF) \cite{silva2025dmmf}, which demonstrated that cognitive patterns could be systematically inferred from software artifacts such as commits, code structure, and documentation. Analyzing 40 repositories and 2,799 commits over three years, DMMF revealed how static artifacts encode developer cognition through four dimensions: cognitive (problem-solving patterns), affective (emotional traces in code), conative (motivational indicators), and reflective (self-awareness in documentation). However, DMMF captured only the \textit{output} of cognitive processes---the committed code---not the \textit{process} itself.

This paper extends DMMF from artifact analysis to interaction analysis. We ask: \textbf{What cognitive patterns emerge when we observe not the products of development, but the moment-to-moment collaboration between developer and AI?}
